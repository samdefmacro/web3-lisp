...
(coalton-toplevel
  ...
  (define (decode-erc20-transfer log)
    ...
    (match (.topics log)
      ((Cons _ (Cons from-topic (Cons to-topic (Nil))))
       (do (from-addr <- (decode-indexed-address from-topic))
           (to-addr   <- (decode-indexed-address to-topic))
           (decoded   <- (abi:abi-decode (Cons (abi:AbiUint 256) Nil) (.data log)))
           (match decoded
  ...
  (define (decode-erc20-approval log)
    ...
    (match (.topics log)
      ((Cons _ (Cons owner-topic (Cons spender-topic (Nil))))
       (do (owner-addr   <- (decode-indexed-address owner-topic))
           (spender-addr <- (decode-indexed-address spender-topic))
           (decoded      <- (abi:abi-decode (Cons (abi:AbiUint 256) Nil) (.data log)))
           (match decoded
             ((Cons (abi:AbiUintVal value) (Nil))
  ...
  (define (decode-erc721-transfer log)
    ...
    (match (.topics log)
      ((Cons _ (Cons from-topic (Cons to-topic (Cons token-id-topic (Nil)))))
       (do (from-addr <- (decode-indexed-address from-topic))
           (to-addr   <- (decode-indexed-address to-topic))
           (token-id  <- (decode-indexed-uint256 token-id-topic))
  ...
  (define (decode-erc1155-transfer-single log)
    ...
    (match (.topics log)
      ((Cons _ (Cons operator-topic (Cons from-topic (Cons to-topic (Nil)))))
       (do (operator-addr <- (decode-indexed-address operator-topic))
           (from-addr     <- (decode-indexed-address from-topic))
           (to-addr       <- (decode-indexed-address to-topic))
           (decoded       <- (abi:abi-decode (Cons (abi:AbiUint 256)
  ...
  (declare %or-unknown ((types:Web3Result DecodedEvent) -> types:Bytes -> DecodedEvent))
  (define (%or-unknown result topic0)
    (match result
      ((Ok decoded) decoded)
      ...
      ((Cons topic0 _)
       (cond
         ((types:bytes-equal? topic0 erc20-transfer-topic)
          ;; 4 topics = ERC-721 (tokenId indexed); 3 topics = ERC-20 (value in data)
          (match (.topics log)
            ((Cons _ (Cons _ (Cons _ (Cons _ (Nil)))))
             (%or-unknown (decode-erc721-transfer log) topic0))
            (_
             (%or-unknown (decode-erc20-transfer log) topic0))))
         ((types:bytes-equal? topic0 erc20-approval-topic)
          (%or-unknown (decode-erc20-approval log) topic0))
         ((types:bytes-equal? topic0 erc1155-transfer-single-topic)
          (%or-unknown (decode-erc1155-transfer-single log) topic0))
