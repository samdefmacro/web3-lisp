...
(defun run-erc1155-tests ()
  ...
  (test-case "selector uri(uint256) = 0x0e89341c"
    (let ((selector (coalton:coalton (web3/erc1155:selector-uri coalton:Unit))))
  ...
  (test-case "selector balanceOf(address,uint256) = 0x00fdd58e"
    (let ((selector (coalton:coalton (web3/erc1155:selector-balance-of coalton:Unit))))
  ...
  (test-case "selector balanceOfBatch(address[],uint256[]) = 0x4e1273f4"
    (let ((selector (coalton:coalton (web3/erc1155:selector-balance-of-batch coalton:Unit))))
  ...
  (test-case "selector isApprovedForAll(address,address) = 0xe985e9c5"
    (let ((selector (coalton:coalton (web3/erc1155:selector-is-approved-for-all coalton:Unit))))
  ...
  (test-case "selector safeTransferFrom(address,address,uint256,uint256,bytes) = 0xf242432a"
    (let ((selector (coalton:coalton (web3/erc1155:selector-safe-transfer-from coalton:Unit))))
  ...
  (test-case "selector safeBatchTransferFrom(...) = 0x2eb2c2d6"
    (let ((selector (coalton:coalton (web3/erc1155:selector-safe-batch-transfer-from coalton:Unit))))
  ...
  (test-case "selector setApprovalForAll(address,bool) = 0xa22cb465"
    (let ((selector (coalton:coalton (web3/erc1155:selector-set-approval-for-all coalton:Unit))))
  ...
  (test-case "erc1155-safe-transfer-from-data builds correct calldata"
    (let* ((from-addr (result-value (coalton:coalton
           ...
           (data (coalton:coalton (web3/types:bytes-empty coalton:Unit)))
