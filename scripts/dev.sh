#!/usr/bin/env bash
# cl-workbench-template: dev.sh.template template=93975c57c28d rendered=0aca10736b86 version=0.13.0-dev
...
REQUIRED_PATHS=()
# Host environment variables forwarded into the warm container and every eval
# when they are set (project switches such as an RPC URL or an integration
# flag). Unset variables are not forwarded, so a container never sees an empty
# override.
...
# Prints `--env NAME` pairs (one per line) for every PASSTHROUGH_ENV variable
# that is set in this shell; `identity passthrough-env` lists the names.
...
Commands:
  ...
  identity FIELD     Print checkout-id|session-id|container|image|port|fasl-volume
                     |passthrough-env (the forwarded variables currently set)
...
identity_field() {
  ...
  case "$1" in
    ...
    passthrough-env) passthrough_env_names ;;
